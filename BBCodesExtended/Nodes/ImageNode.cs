...
namespace BBCodesExtended.Nodes
{
    ...
    public class ImageNode : Node
    {
        ...
        public override string ToHTML()
        {
            ...
            {
                string ret = "<img src=\"" + EncodeForHtmlAttribute(innerContent, true) + "\" alt=\"" + EncodeForHtmlAttribute(innerContent, true) + "\" ";
            ...
            if (Arguments != null && Arguments.Count == 1)
            {
                if(!string.IsNullOrWhiteSpace(this.Arguments[0].Item1))
                {
                    ...
                    {
                        return "<img src=\"" + EncodeForHtmlAttribute(innerContent, true) + "\" alt=\"" + EncodeForHtmlAttribute(innerContent, true) + "\" width=\"" + EncodeForHtmlAttribute(dimensions[0]) + "\" height=\"" + EncodeForHtmlAttribute(dimensions[1]) + "\" />";
                    }
                }
            }

            // No width and height specified
            return "<img src=\"" + EncodeForHtmlAttribute(innerContent, true) + "\" alt=\"" + EncodeForHtmlAttribute(innerContent, true) + "\" />";
        }
